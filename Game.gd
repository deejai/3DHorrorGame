...
func _on_callout_trigger_1_body_entered(body):
    if body is Player:
        GameStateTracker.trigger_event(GameStateTracker.TriggeredEvent.INTRO_CALL_MADDY_1, $CalloutTrigger1)


func _on_callout_trigger_2_body_entered(body):
    if body is Player:
        GameStateTracker.trigger_event(GameStateTracker.TriggeredEvent.INTRO_CALL_MADDY_2, $CalloutTrigger2)


func _on_callout_trigger_3_body_entered(body):
    if body is Player:
        GameStateTracker.trigger_event(GameStateTracker.TriggeredEvent.INTRO_CALL_MADDY_3, $CalloutTrigger3)


func _on_callout_trigger_4_body_entered(body):
    if body is Player:
        GameStateTracker.trigger_event(GameStateTracker.TriggeredEvent.INTRO_CALL_MADDY_4, $CalloutTrigger4)


func _on_first_encounter_trigger_body_entered(body):
    if body is Player:
        GameStateTracker.trigger_event(GameStateTracker.TriggeredEvent.GHOST_ENTERS_HOME_FIRST_TIME, $FirstEncounterTrigger)
